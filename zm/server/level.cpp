    ...
    jm(jsonMap), camera(players)
{
  ...
  for (auto&& spawn : jm.spawnsData) {
    ...
    if ( type == BUMBY ) {
      ...
      enemy = new Bombman(physics, x, y, jm);
    } else if ( type == FIREMAN ) {
      enemy = new Fireman(physics, x, y, jm);
    } else if ( type == MAGNETMAN ) {
      enemy = new Magnetman(physics, x, y, jm);
    } else if ( type == SPARKMAN ) {
      enemy = new Sparkman(physics, x, y, jm);
    } else if ( type == RINGMAN ) {
      enemy = new Ringman(physics, x, y, jm);
    }
